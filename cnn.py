...
def load_data(data_path):
    ...
    with open(data_path, "r") as fp:   #open jason file to read & load data from in variable data
        data = json.load(fp)

    X = np.array(data["mfcc"])         #save mfcc features of records in X
    y = np.array(data["labels"])       #save labels of each record in y
    return X, y


def plot_history(history):
    """
    draw accuracy/loss for training/validation set as a function of the epochs
        history ---> Training history of model
    """

    fig, axs = plt.subplots(2)         #drawing 2 figures (accuracy, error)
